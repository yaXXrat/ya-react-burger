    ...
    return (
      <div className={classNames(style['feed-order-block'])} >
          ...
          <div className={style['feed-order-short'] + ' text_type_main-medium'}>
              ...
              { currentOrder.status === 'done' && <div className={style.status + ' text_type_main-default '+ style.done}>Выполнен</div>}
              { currentOrder.status === 'created' && <div className={style.status + ' text_type_main-default'}>Создан</div>}
              { currentOrder.status === 'pending' && <div className={style.status + ' text_type_main-default'}>Готовится</div>}
              { currentOrder.status === 'cancelled' && <div className={style.status + ' text_type_main-default '+ style.cancel}>Отменен</div>}
